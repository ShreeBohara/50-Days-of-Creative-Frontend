...
function setCardTilt(card, event) {
  ...
  const magnetX = (relativeX - 0.5) * 8;
  const magnetY = (relativeY - 0.5) * 6;

  card.style.setProperty("--card-rotate-x", `${rotateX.toFixed(2)}deg`);
  card.style.setProperty("--card-rotate-y", `${rotateY.toFixed(2)}deg`);
  card.style.setProperty("--card-translate-x", `${magnetX.toFixed(2)}px`);
  card.style.setProperty("--card-translate-y", `${magnetY.toFixed(2)}px`);
