...
// Polite, screen-reader-only announcements for figure changes.
export default function LiveAnnouncer() {
  ...
  useEffect(() => {
    ...
    setMessage(`Figure drawn${ticks.current % 2 ? '' : ' '}`)
  }, [drawKey])

  useEffect(() => {
    if (count > prevCount.current) setMessage('Figure saved to collection')
